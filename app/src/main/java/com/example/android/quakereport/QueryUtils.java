...
public final class QueryUtils {
     ...
     * NO ONE WILL BE ABLE TO CREATE AN INSTANCE OF THIS CLASS
     */
    private QueryUtils() {
    }
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // Getting JSON Array node root
            JSONObject jsonObjRoot = new JSONObject(SAMPLE_JSON_RESPONSE);

            // Getting JSON Array node features
            JSONArray features = jsonObjRoot.getJSONArray("features");

            // looping through All features
            for (int i = 0; i < features.length(); i++) {

                //get the position of each features in the array
                JSONObject currentArrayPosition = features.getJSONObject(i);

                //get the JSON Object properties in current position
                JSONObject properties = currentArrayPosition.getJSONObject("properties");
